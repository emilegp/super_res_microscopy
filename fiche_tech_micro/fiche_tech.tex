...
C'est ici que le traitement de toutes les images est important, même celles qui sont "perdues", car sinon le $\Delta t$ n'est pas cohérent avec la procédure de la MSD \cite{rouillon_travail_2024}. 
La façon de traiter ce calcul avec les vecteurs \texttt{[NaN, NaN]} tout en perdant le moins d'information possible a été de définir qu'au moment où l’élément \texttt{NaN} est détecté dans un calcul, $\left( \mathbf{r}(i+\Delta t) - \mathbf{r}(i) \right)=0$. De cette façon, les images non-enregistrées
ne peuvent pas interférer dans le calcul de la MSD. Dans la même idée, les incertitudes sur les positions sont calculées à l'aide de l'écart-type 
du \textit{fit} gaussien, et dans le cas d'une vecteur identifié par \texttt{[NaN, NaN]}, on pose $\Delta x=0$ et $\Delta y=0$. Les images noires ne sont donc pas prises en compte non plus dans les calculs d'incertitude. 
...
Pour un échantillon contenant des particule de 1$\mu m$, 9 particules ont été suivies pour déterminer, par une analyse statistique, leurs rayons ainsi que leurs coefficients de diffusion. 
Le meme principe a été utilisé dans un échantillon contenant des particule de 10$\mu m$, dans ce cas ci, 3 particules on été jugé comme analysable. 

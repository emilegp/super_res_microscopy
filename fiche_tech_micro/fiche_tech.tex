...
du fit gaussien, dans le cas d'une image noire, on pose $\Delta x=0$ et $\Delta y=0$. Les images noir ne sont donc pas pris en compte non plus dans les calcul d'incertitude.  

Une fois toutes les valeurs trouvées, un graphique des résultat de MDS en fonction du $\Delta t$ est produit. Afin de bien caractériser le mouvement de la particule
qui correspond au mouvement brownien, un \textit{fit} quadratique sur les points de la MSD est déterminé; les 5 premiers points sont choisis. En utilisant un \textit{fit} quadratique et non un \textit{fit} linéaire,
...
il a été conclu qu'une magnification de 20 offrait un plage de fonctionnement optimale, en considérant 

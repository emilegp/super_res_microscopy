...
\begin{table}[H]
  ...
  \hline
      ...
      $17.14\pm0.02$ & \textcolor{red}{XX} & L 600 $\times$ W 90 $\times$ H 200 & \$1,483.59 & \vspace{-20pt} {\small\parbox{7cm}{\setlength{\columnsep}{0pt} 
...
déterminée sur au moins une cinquantaine de frames successifs, avant de pouvoir procéder à l'estimation de son 
coefficient de diffusion et de sa taille. 

Pour reconnaitre la particule, système de comparaison en fonction de sa taille (facteur majeur) et sa luminosité 
(impact moindre, mais présent) est utilisé lorsque plus d'une particule sont dans la même zone. 
OBTENTION DE CES INFOS ET MÉCANISME DE COMPARAISON.
...
et sont d'environ 100 pixels de large et 50 sur un maximum de 200 respectivement.
...
La position de la particule est alors donné par le fit et l'incertitude sur la position correspond alors à 
l'écart-type de la gaussienne obtenue, qui est généralement plus faible que la taille d'un pixel. 
Des vérifications sont faites pour s'assurer que la nouvelle position est plausible et la nouvelle position est alors
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{Test_cond_rapport.png}

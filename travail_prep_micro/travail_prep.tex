...
Le système optique utlisé pour la conception du microscope est illustré de manière simplifiée
...
est une fonction de Bessel. L'émission des photons dans le temps s'effectue en suivant une distribution de Poisson:
\begin{equation}
...
d'émission choisi à partir de \textit{np.random.poisson} et d'un $\lambda$ arbitraire. La position de chaque photon
émis durant la simulation est posée en fonction de la distribution de probabilité imposée par la fonction d'étalement 
de la particule ($PSF$).
...
d'une gaussienne 2D a été effectuée avec la librairie $lmfit$. La fonction gaussienne 2D est définie par:
\begin{equation}

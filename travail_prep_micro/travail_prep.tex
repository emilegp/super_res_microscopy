...
\textcolor{red}{MSD a optimisé??(partie philou)}

Une fois chaque position obtenue, il est possible de trouver la valeur du coefficient de diffusion pour la particule. 
Pour ce faire il est nécessaire d'effectuer le calcul du déplacement quadratique moyen. On réalise ce calcul en faisant la moyenne de déplacement de même intervalle. Par exemple, 
la première moyenne sera calculée avec toutes les distances séparant deux positions éloignées à intervalle de $\Delta t$. De la même manière, la deuxième moyenne sera calculée avec toutes les distances
entre deux point placés a $2 \Delta t$ d'intervalle. On peut modéliser ce calcul par la formule suivante: 
\begin{equation}
  MSD(d) = \frac{1}{N_d} \sum_{i=0}^{N_d - 1} \left( \mathbf{r}(i+d) - \mathbf{r}(i) \right)^2
\end{equation}
Avec $N_d$ qui est le nombre de pairs i disponible pour un intervalle d et d qui varient de 1 jusqu'au nombre de positions-1. 
Une fois les valeurs des MDS trouvés pour chaque déplacement de même intervalle, il est possible de généré un graphique 
des MDS en fonction de la valeur de l'intervalle d. Sur ce graphique, seuls les premiers points seront sélectionnés, c'est à dire pour des d faible.
Cette procédure est nécessaire pour observée le réel comportement du mouvement, car plus d est petit plus il y a de paire de points disponible. 
...
ces points, c'est la pente de cette régression qui nous donnera la valeur du coefficient de diffusion estimé par notre microscope. 
